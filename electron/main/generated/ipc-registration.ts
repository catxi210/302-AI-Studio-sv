import { ipcMain } from 'electron';
		import { registryService, broadcastService, storageService, pluginService, generalSettingsService, codeAgentService, ghostWindowService, windowService, shortcutService, tabService, aiApplicationService, appService, dataService, externalLinkService, mcpService, ssoService, threadService, updaterService } from '../services';
...
ipcMain.handle('registryService:clearCache', (event) =>
	registryService.clearCache(event)
);
ipcMain.handle('registryService:getCacheInfo', (event) =>
	registryService.getCacheInfo(event)
);
...
ipcMain.handle('storageService:getItem', (event, key) =>
	storageService.getItem(event, key)
);
ipcMain.handle('storageService:hasItem', (event, key) =>
	storageService.hasItem(event, key)
);
ipcMain.handle('storageService:removeItem', (event, key, options) =>
	storageService.removeItem(event, key, options)
);
ipcMain.handle('storageService:getKeys', (event, base) =>
	storageService.getKeys(event, base)
);
ipcMain.handle('storageService:clear', (event, base) =>
	storageService.clear(event, base)
);
ipcMain.handle('storageService:getMeta', (event, key) =>
	storageService.getMeta(event, key)
);
...
ipcMain.handle('codeAgentService:updateClaudeCodeSandboxModel', (event, threadId, sandbox_id, llm_model) =>
...
ipcMain.handle('codeAgentService:updateClaudeCodeCurrentSessionIdByThreadId', (event, threadId, sessionId) =>
...
ipcMain.handle('codeAgentService:updateClaudeCodeSessionRemark', (event, sandbox_id, session_id, remark) =>
...
ipcMain.handle('windowService:handleMoveTabIntoExistingWindow', (event, triggerTabId, windowId, insertIndex) =>
...
ipcMain.handle('shortcutService:getConflicts', (event) =>
	shortcutService.getConflicts(event)
);
ipcMain.handle('shortcutService:getSyncInfo', (event) =>
	shortcutService.getSyncInfo(event)
);
...
ipcMain.handle('tabService:handleNewTab', (event, title, type, active, href, content, previewId) =>
...
ipcMain.handle('tabService:getActiveTab', (event) =>
	tabService.getActiveTab(event)
);
ipcMain.handle('tabService:getAllTabsForCurrentWindow', (event) =>
	tabService.getAllTabsForCurrentWindow(event)
);
ipcMain.handle('tabService:getAllTabs', (event) =>
	tabService.getAllTabs(event)
);
...
ipcMain.handle('tabService:handleTabCloseOffside', (event, tabId, tabIdsToClose, _remainingTabIds, shouldSwitchActive) =>
	tabService.handleTabCloseOffside(event, tabId, tabIdsToClose, _remainingTabIds, shouldSwitchActive)
);
...
ipcMain.handle('appService:getTheme', (event) =>
	appService.getTheme(event)
);
ipcMain.handle('appService:setTheme', (event, theme) =>
	appService.setTheme(event, theme)
);
ipcMain.handle('appService:restartApp', (event) =>
	appService.restartApp(event)
);
ipcMain.handle('appService:resetAllData', (event) =>
	appService.resetAllData(event)
);
ipcMain.handle('appService:clearChatHistory', (event) =>
	appService.clearChatHistory(event)
);

// dataService service registration
ipcMain.handle('dataService:importLegacyJson', (event) =>
	dataService.importLegacyJson(event)
);
ipcMain.handle('dataService:exportStorage', (event) =>
	dataService.exportStorage(event)
);
ipcMain.handle('dataService:importStorage', (event) =>
	dataService.importStorage(event)
);
ipcMain.handle('dataService:listBackups', (event) =>
	dataService.listBackups(event)
);
...
ipcMain.handle('ssoService:cancelSsoLogin', (event) =>
	ssoService.cancelSsoLogin(event)
);
...
ipcMain.handle('threadService:getThreads', (event) =>
	threadService.getThreads(event)
);
